...
class dog:
        ...
        if len(self.path) != 0:
            if mousepoweractivated.gettime() > 45:
                ...
                if self.x < int(self.path[0][0]) + 0.475:
                    ...
                    self.x += dgeschw * 60 / (clock.get_fps() + 0.00000001)
                ...
                if self.y < int(self.path[0][1]) + 0.475:
                    self.fastrichtung += 2
                    self.anzahlrichtungen += 1
                    self.y += dgeschw * 60 / (clock.get_fps() + 0.00000001)
    ...
    def sleep_draw(self):
        ...
        if zoom and legit and not inhole:
            screen.blit(
                pygame.transform.rotate(pygame.transform.scale(sldogimg, (ZOOM_FB, ZOOM_FB)), self.richtung * 90),
                ((2.5 + (self.x - x)) * ZOOM_FB + BreiVer
...
def textures():
    global kitty, wall, heartimg, path, glowf, deadf, dogimg, dogsound, winsound, diesound, damagesound, holeimg, mouseimg, sldogimg, other_cat
    try:
        kitty = pygame.image.load("texturepacks/" + texturepack + "/kitty.png")
    except:
        kitty = pygame.image.load("texturepacks/default/kitty.png")
    try:
        other_cat = pygame.image.load("texturepacks/" + texturepack + "/other_cat.png")
    except:
        other_cat = pygame.image.load("texturepacks/default/other_cat.png")
    try:
    ...
    try:
        glowf = [pygame.image.load("texturepacks/" + texturepack + "/glowing_fish/1.png"),
                 pygame.image.load("texturepacks/" + texturepack + "/glowing_fish/2.png")]
    except:
        glowf = [pygame.image.load("texturepacks/default/glowing_fish/1.png"),
                 pygame.image.load("texturepacks/default/glowing_fish/2.png")]
    try:
...
def reset():
    global l, x, y, hearts, fish, feld, fat, geschw, startx, starty, holes, mice, dogs, whereru, howmanydogs, dogpos, dogrichtungen, savedas, dgeschw
    fat = 0
    ...
    for _ in range(1000):
        ...
        if doeselementinlistexist(lab, cur + 1) and richtung == 'o' and lab[cur + 1] == 'e' and (cur + 1) % width != 0:
            ...
            if doeselementinlistexist(lab, cur + width - 1) and cur + width - 1 >= 0 and lab[cur + width - 1] == 'e':
                lab[cur + width - 1] = 'w' + str(cnt)
            verbotene_richtungen = []
        elif doeselementinlistexist(lab, cur + width) and richtung == 's' and lab[cur - width] == 'e' and (
                cur // width) < height:
            ...
            if doeselementinlistexist(lab, cur - width + 1) and cur - width + 1 >= 0 and lab[cur - width + 1] == 'e':
                lab[cur - width + 1] = 'w' + str(cnt)
            verbotene_richtungen = []
        elif doeselementinlistexist(lab, cur - 1) and richtung == 'w' and lab[cur - 1] == 'e' and (
                cur - 1) % width != width - 1:
            lab[cur - 1] = 'p' + str(cnt)
            path_fields.append(cur - 1)
            ...
            verbotene_richtungen = []
        # nach norden
        elif doeselementinlistexist(lab, cur - width) and richtung == 'n' and lab[
            cur - width] == 'e' and cur // width > 0:
            lab[cur - width] = 'p' + str(cnt)
            path_fields.append(cur - width)
    ...
    if legit and zoom and not inhole and whereru != "tut":
        if mousepoweractivated.gettime() < 45:
            screen.blit(pygame.transform.rotate(pygame.transform.scale(kitty, (
                (ZOOM_FB + fat * 3) * (mousepoweractivated.gettime() / 45),
                ZOOM_FB * (mousepoweractivated.gettime() / 45))),
        ...
        if mousepoweractivated.gettime() < 45:
            screen.blit(pygame.transform.rotate(pygame.transform.scale(kitty, (
            (FB + fat * 3) * (mousepoweractivated.gettime() / 45), FB * (mousepoweractivated.gettime() / 45))),
                                                richtung * 90), (x * FB + BreiVer - (pygame.transform.rotate(
                pygame.transform.scale(kitty, (
                (FB + fat * 3) * (mousepoweractivated.gettime() / 45), FB * (mousepoweractivated.gettime() / 45))),
                richtung * 90).get_width()) // 2, y * FB + HohVer - (pygame.transform.rotate(
                pygame.transform.scale(kitty, (
                (FB + fat * 3) * (mousepoweractivated.gettime() / 45), FB * (mousepoweractivated.gettime() / 45))),
                richtung * 90).get_height()) / 2))
        else:
...
def getPlayerIndexById(id, players):
...
dogsavingvariable = ""
savingvariable = ""
savinglist = ["l", "x", "y", "hearts", "fish", "feld", "fat", "geschw", "startx", "starty", "howmanydogs", "holes",
              "mice", "dogs", "dogpos", "dogrichtungen"]
...
onslider = 0
waspressed = False
multiplayerdead = False


def damage():
    global x, y, starty, startx, hearts, whereru, multiplayerdead
    if whereru == "serverplay":
        hearts -= 1
        x, y = getPlayerById(data[2], data[1]).startx, getPlayerById(data[2], data[1]).starty
        if hearts > 0:
            plymusic(damagesound, 1, False)
        if hearts < 1:
            plymusic(diesound, 1, False)
            multiplayerdead = True
    else:
        hearts -= 1
        x, y = startx, starty
        if hearts > 0:
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if whereru == "onlinescreen":
                ...
                if event.key == pygame.K_BACKSPACE:
                    name = name[:-2]
            if event.key == pygame.K_r and whereru == "play":
                reset()
            if event.key == pygame.K_ESCAPE and (
                    whereru == "play" or whereru == "tut") and timebetweenpauseandplay > 10:
                whereru = "pause"
                timebetweenpauseandplay = 0
                pausewru = "main"
    ...
    if whereru == "onlinescreen":
        ...
        for i in range(len(servers)):
            if selectedworld == i:
                button(5, i * (100 + 5) + scrollverschiebung, 500, 100,
                       ...
                       servers[i][0],
                       "global whereru,activeserver,server,data;  activeserver = servers[i][1];data = server.connect(name,activeserver);whereru = 'serverplay'",
                       50)
        ...
        if selectedworld != -1:
            ...
            button(20 + (pxl_width - 30) / 2, pxl_height - pxl_height / 1.85, (pxl_width - 30) / 2, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font,
                   "Edit",
                   "global whereru,servername,serverip,edit;whereru = 'addserver';servername = servers[i][0];serverip = servers[i][1];edit = selectedworld",
                   80)
        ...
        namewrite = pygame.font.SysFont(font, 80).render(name, True, (color))
        pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]),
                         (20 + (pxl_width - 30) / 2, pxl_height - pxl_height / 2.5, (pxl_width - 30) / 2, 100))
        screen.blit(namewrite, ((20 + (pxl_width - 30) / 2) + (((pxl_width - 30) / 2) - namewrite.get_width()) // 2,
                                pxl_height - pxl_height / 2.5 + (100 - namewrite.get_height()) // 2))
        # x + (width - writething.get_width()) // 2
    ...
    if whereru == "addserver":
        ...
        if servername == "":
            writething = pygame.font.SysFont(font, 90).render('(Name)', True,
                                                              (color))
        else:
            writething = pygame.font.SysFont(font, 90).render(servername, True,
                                                              (color))
        if serverip == "":
            writething1 = pygame.font.SysFont(font, 90).render('(IP)', True,
                                                               (color))
        else:
            writething1 = pygame.font.SysFont(font, 90).render(serverip, True,
                                                               (color))
        pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]), (
        (pxl_width - writething.get_width()) // 2 - 5, (pxl_height - writething.get_height()) // 5 - 5,
        writething.get_width() + 10, writething.get_height() + 10))
        screen.blit(writething,
                    ((pxl_width - writething.get_width()) // 2, (pxl_height - writething.get_height()) // 5))
        pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]), (
        (pxl_width - writething1.get_width()) // 2 - 5, (pxl_height - writething1.get_height()) // 2.5 - 5,
        writething1.get_width() + 10, writething1.get_height() + 10))
        screen.blit(writething1,
                    ((pxl_width - writething1.get_width()) // 2, (pxl_height - writething1.get_height()) // 2.5))
        ...
        button(20 + (pxl_width - 30) / 2, pxl_height - pxl_height / 5, (pxl_width - 30) / 2, 100,
               (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font,
               "Save",
               "global whereru,servers,edit; whereru = 'onlinescreen'\nif edit != -1: servers[edit] = [servername,serverip]\nelse: servers.append([servername,serverip])",
               80)
        stringthatwillbewritten = "["
        for i in servers:
            stringthatwillbewritten += "["
    ...
    if whereru == "serverplay":
        ...
        if data[0] == 0:
            ...
            anzahlrichtungen = 0
            key = pygame.key.get_pressed()
            x = getPlayerById(data[2], data[1]).x
            y = getPlayerById(data[2], data[1]).y
            fat = getPlayerById(data[2], data[1]).fat
            yourtime = data[4]
            dogs = data[5]
            dogpos = data[6]
            dogrichtungen = data[7]
            holes = data[8]
            fish = data[9]
            mice = data[10]
            ...
            if (key[pygame.K_w] or key[pygame.K_UP]) and y >= 0:
                fastrichtung += 0
                anzahlrichtungen += 1
                y -= (geschw - 0.001 * fat) * 60 / (clock.get_fps() + 0.00000001)
            if (key[pygame.K_a] or key[pygame.K_LEFT]) and x >= 0:
                fastrichtung += 1
                anzahlrichtungen += 1
                x -= (geschw - 0.001 * fat) * 60 / (clock.get_fps() + 0.00000001)
            if (key[pygame.K_s] or key[pygame.K_DOWN]) and y < height - (geschw - 0.001 * fat) * 60 / (
                    clock.get_fps() + 0.00000001):
                fastrichtung += 2
                anzahlrichtungen += 1
                y += (geschw - 0.001 * fat) * 60 / (clock.get_fps() + 0.00000001)
            if (key[pygame.K_d] or key[pygame.K_RIGHT]) and x < width - (geschw - 0.001 * fat) * 60 / (
                    clock.get_fps() + 0.00000001):
                fastrichtung += 3
                anzahlrichtungen += 1
                x += (geschw - 0.001 * fat) * 60 / (clock.get_fps() + 0.00000001)
                ...
                if zoom:
                    if not inhole:
                        ...
                        for j in range(len(l)):
                            if (j % width) < x + 3 and (j % width) > x - 4 and (j // width) < y + 3 and (
                                    j // width) > y - 4:
                                ...
                                if l[j][0] == "w":
                                    screen.blit(pygame.transform.scale(wall, (ZOOM_FB, ZOOM_FB)), (
                                        col_idx * ZOOM_FB + BreiVer + (int(x) - x + 0.5) * ZOOM_FB,
                                        row_idx * ZOOM_FB + HohVer + (int(y) - y + 0.5) * ZOOM_FB))
                                    col_idx += 1

                                if l[j][0] == "p":
                                    # pygame.draw.rect(screen, (0, 0, 255), (col_idx, row_idx, FB, FB))
                                    screen.blit(pygame.transform.scale(path, (ZOOM_FB, ZOOM_FB)), (
                                        col_idx * ZOOM_FB + BreiVer + (int(x) - x + 0.5) * ZOOM_FB,
                                        row_idx * ZOOM_FB + HohVer + (int(y) - y + 0.5) * ZOOM_FB))
                                    col_idx += 1
                                if col_idx == 6:
                        ...
                        for j in range(len(l)):
                            if fish.count(j) != 0:
                                if fish.index(j) != 0 and (j % width) < x + 2 and (j % width) > x - 3 and (
                                    ...
                                    screen.blit(pygame.transform.scale(deadf, (ZOOM_FB, ZOOM_FB)), (
                                        (2.5 + (j % width - x)) * ZOOM_FB + BreiVer,
                                        (2.5 + (j // width - y)) * ZOOM_FB + HohVer))
                        for j in range(len(l)):
                            if holes.count(j) != 0 and (j % width) < x + 2 and (j % width) > x - 3 and (
                                    j // width) < y + 2 and (j // width) > y - 3:
                                screen.blit(pygame.transform.scale(holeimg, (ZOOM_FB, ZOOM_FB)), (
                                    (2.5 + (j % width - x)) * ZOOM_FB + BreiVer,
                                    (2.5 + (j // width - y)) * ZOOM_FB + HohVer))
                        ...
                        for j in range(len(l)):
                            ...
                            if mice.count(j) != 0 and (j % width) < x + 2 and (j % width) > x - 3 and (
                                    j // width) < y + 2 and (
                                    j // width) > y - 3:
                                screen.blit(pygame.transform.scale(mouseimg, (ZOOM_FB, ZOOM_FB)), (
                                    (2.5 + (j % width - x)) * ZOOM_FB + BreiVer,
                                    (2.5 + (j // width - y)) * ZOOM_FB + HohVer))
                        ...
                        pygame.draw.circle(screen, (color[0], color[1], color[2]),
                                           (2.5 * ZOOM_FB + BreiVer, 2.5 * ZOOM_FB + HohVer), 4.5 * ZOOM_FB,
                                           int(2.5 * ZOOM_FB))
                        for j in range(len(l)):
                            row_idx = (j // width) * FB + HohVer
                            col_idx = (j % width) * FB + BreiVer
                        ...
                        pygame.draw.rect(screen, (0, 0, 0),
                                         (pxl_width - ZOOM_FB - 10, pxl_height - ZOOM_FB - 10, ZOOM_FB, ZOOM_FB), 0, 4,
                                         4, 4, 4)
                        pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]),
                                         (pxl_width - ZOOM_FB - 10, pxl_height - ZOOM_FB - 10, ZOOM_FB, ZOOM_FB), 3, 4,
                                         4, 4, 4)
                        for i in data[1]:
                            pygame.draw.circle(screen, (255, 255, 255),
                                               (pxl_width - ZOOM_FB - 10 + i.x / width * ZOOM_FB,
                                                pxl_height - ZOOM_FB - 10 + i.y / width * ZOOM_FB),
                                               max(ZOOM_FB / 20, 5))
                        for i in dogpos:
                            pygame.draw.circle(screen, (255, 0, 0), (pxl_width - ZOOM_FB - 10 + i[0] / width * ZOOM_FB,
                                                                     pxl_height - ZOOM_FB - 10 + i[
                                                                         1] / width * ZOOM_FB), max(ZOOM_FB / 30, 5))
                        ...
                        for j in range(len(l)):
                            ...
                            if fish.count(j) != 0:
                                if fish.index(j) == 0:
                                    if (j % width) < x + 4 and (j % width) > x - 5 and (j // width) < y + 4 and (
                                            j // width) > y - 5:
                                        screen.blit(pygame.transform.scale(glowf[fishmodel], (FB, FB)),
                                                    (col_idx, row_idx))
            ...
            if legit and zoom and not inhole:
                ...
                for i in data[1]:
                    if i.id != data[2]:
                        if i.mousepoweractivated.gettime() < 45:
                            screen.blit(
                                pygame.transform.rotate(pygame.transform.scale(other_cat, (
                                (ZOOM_FB + i.fat * 3) * (i.mousepoweractivated.gettime() / 45),
                                ZOOM_FB * (i.mousepoweractivated.gettime() / 45))),
                                                        i.richtung * 90),
                                ((2.5 + (i.x - x)) * ZOOM_FB + BreiVer
                                 - (pygame.transform.rotate(pygame.transform.scale(other_cat, (
                                (ZOOM_FB + i.fat * 3) * (i.mousepoweractivated.gettime() / 45),
                                ZOOM_FB * (i.mousepoweractivated.gettime() / 45))),
                                                            i.richtung * 90).get_width()) // 2,
                                 (2.5 + (i.y - y)) * ZOOM_FB + HohVer - (
                                     pygame.transform.rotate(pygame.transform.scale(other_cat, (
                                     (ZOOM_FB) + i.fat * 3 * (i.mousepoweractivated.gettime() / 45),
                                     ZOOM_FB * (i.mousepoweractivated.gettime() / 45))),
                                                             i.richtung * 90)
                            ...
                            screen.blit(
                                pygame.transform.rotate(
                                    pygame.transform.scale(other_cat, (ZOOM_FB + i.fat * 3, ZOOM_FB)), i.richtung * 90),
                                ((2.5 + (i.x - x)) * ZOOM_FB + BreiVer
                                 - (pygame.transform.rotate(
                                    pygame.transform.scale(other_cat, (ZOOM_FB + i.fat, ZOOM_FB)),
                                    i.richtung * 90).get_width()) // 2,
                                 (2.5 + (i.y - y)) * ZOOM_FB + HohVer - (
                                     pygame.transform.rotate(
                                         pygame.transform.scale(other_cat, (ZOOM_FB + i.fat * 3, ZOOM_FB)),
                                         i.richtung * 90)
                                     .get_height()) / 2))
                ...
                if mousepoweractivated.gettime() < 45:
                    screen.blit(pygame.transform.rotate(pygame.transform.scale(kitty, (
                        ...
                        ZOOM_FB * (mousepoweractivated.gettime() / 45))),
                                                        richtung * 90),
                                (2.5 * ZOOM_FB + BreiVer - (pygame.transform.rotate(
                                    pygame.transform.scale(kitty, (
                                        (ZOOM_FB + fat * 3) * (mousepoweractivated.gettime() / 45),
                                        ZOOM_FB * (mousepoweractivated.gettime() / 45))),
                                    richtung * 90).get_width()) // 2, 2.5 * ZOOM_FB + HohVer - (pygame.transform.rotate(
                ...
                # minimap
                pygame.draw.rect(screen, (0, 0, 0),
                                 (pxl_width - ZOOM_FB - 10, pxl_height - ZOOM_FB - 10, ZOOM_FB, ZOOM_FB), 0,
                                 4,
                                 4, 4, 4)
                pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]),
                                 (pxl_width - ZOOM_FB - 10, pxl_height - ZOOM_FB - 10, ZOOM_FB, ZOOM_FB), 3,
                                 4,
                                 4, 4, 4)
                for i in data[1]:
                    if i.id != data[2]:
                ...
                for i in dogpos:
                    pygame.draw.circle(screen, (255, 0, 0),
                                       (pxl_width - ZOOM_FB - 10 + i[0] / width * ZOOM_FB,
                                        pxl_height - ZOOM_FB - 10 + i[
                                            1] / width * ZOOM_FB), max(ZOOM_FB / 30, 5))
                ...
                for i in data[1]:
                    if i.id != data[2]:
                        if i.mousepoweractivated.gettime() < 45:
                            screen.blit(pygame.transform.rotate(pygame.transform.scale(other_cat, (
                                (FB + i.fat * 3) * (i.mousepoweractivated.gettime() / 45),
                                FB * (i.mousepoweractivated.gettime() / 45))),
                                                                richtung * 90),
                                        (i.x * FB + BreiVer - (pygame.transform.rotate(
                                            pygame.transform.scale(other_cat, (
                                                (FB + i.fat * 3) * (i.mousepoweractivated.gettime() / 45),
                                                FB * (i.mousepoweractivated.gettime() / 45))),
                                            i.richtung * 90).get_width()) // 2,
                                         i.y * FB + HohVer - (pygame.transform.rotate(
                                             pygame.transform.scale(other_cat, (
                                                 (FB + i.fat * 3) * (i.mousepoweractivated.gettime() / 45),
                                                 FB * (i.mousepoweractivated.gettime() / 45))),
                                             i.richtung * 90).get_height()) / 2))
                        else:

                            screen.blit(pygame.transform.rotate(pygame.transform.scale(other_cat, (
                                FB + i.fat * 3,
                                FB)), i.richtung * 90), (i.x * FB + BreiVer - (
                                pygame.transform.rotate(pygame.transform.scale(other_cat, (FB + i.fat * 3, FB)),
                                                        i.richtung * 90).get_width()) // 2,
                                                         i.y * FB + HohVer - (
                                                             pygame.transform.rotate(
                                                                 pygame.transform.scale(other_cat, (
                        ...
                        if i.mousepoweractivated.gettime() < 45:
                            ...
                            screen.blit(pygame.transform.rotate(pygame.transform.scale(kitty, (
                                FB + i.fat * 3,
                                FB)), i.richtung * 90), (i.x * FB + BreiVer - (
                                pygame.transform.rotate(pygame.transform.scale(kitty, (FB + i.fat * 3, FB)),
                                                        i.richtung * 90).get_width()) // 2,
                                                         i.y * FB + HohVer - (
                                                             pygame.transform.rotate(
                                                                 pygame.transform.scale(kitty, (
            ...
            if multiplayerdead:
                ...
                tosend = "update " + str(x) + " " + str(y) + " " + str(richtung) + " " + str(fat) + " " + str(
                    mousepoweractivated.startpoint)
        elif data[0] == 1:
            hearts = 7
            # server is starting
        elif data[0] == 2:
            if data[2] != -1:
                for i in range(5):
                    screen.blit(
                        pygame.transform.scale(glowf[fishmodel], ((pxl_width - 20) // 5, (pxl_width - 20) // 5)),
                        ((pxl_width - 20) // 5 * i, (pxl_height - 100) // 2 - (pxl_width - 20) // 5))
                screen.blit(pygame.font.SysFont(font, 50).render(str(data[2]), True,
            ...
            button(10,
                   ...
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font,
                   "Start new round",
                   "global tosend; tosend = 'start'",
                   70)
            playernames = []
            for i in data[1]:
                playernames.append(i.name)
            if len(playernames) == 1:
                screen.blit(pygame.font.SysFont(font, 30).render("One player is currently waiting:", True,
                                                                 (255 - color[0], 255 - color[1], 255 - color[2])),
                            ((pxl_width - pygame.font.SysFont(font, 30).render("One player is currently waiting:", True,
                                                                               (255 - color[0], 255 - color[1],
                                                                                255 - color[2])).get_width()) // 2,
                             pxl_height // 2 + 60))

            if len(playernames) > 1:
                screen.blit(
                    pygame.font.SysFont(font, 30).render(str(len(playernames)) + " players are currently waiting:",
                                                         True,
                                                         (255 - color[0], 255 - color[1], 255 - color[2])), ((
                                                                                                                     ...
                                                                                                                     30).render(
                                                                                                                     str(len(
                                                                                                                         playernames)) + " players are currently waiting:",
                                                                                                                     True,
                                                                                                                     (
                                                                                                                     255 -
                                                                                                                     color[
            ...
            for i in range(len(playernames)):
                if pygame.font.SysFont(font, 20).render(funnystrings[-1] + playernames[i] + ", ", True,
                                                        (255 - color[0], 255 - color[1],
                                                         255 - color[2])).get_width() > pxl_width - 20:
                    funnystrings.append("")
                funnystrings[-1] += playernames[i]
                if i + 1 != len(playernames):
                    funnystrings[-1] += ", "
            for i in range(len(funnystrings)):
                screen.blit(
                    pygame.font.SysFont(font, 20).render(funnystrings[i], True,
                                                         (255 - color[0], 255 - color[1], 255 - color[2])), ((
                                                                                                                     pxl_width - pygame.font.SysFont(
                                                                                                                 font,
                                                                                                                 20).render(
                                                                                                                 ...
                                                                                                                 (255 -
                                                                                                                  ...
                                                                                                                  color[
                                                                                                                      2])).get_width()) // 2,
                                                                                                             pxl_height // 2 + 120 + 40 * i))

        data = server.send(tosend)
    ...
    if whereru == "saveas":
        writething = pygame.font.SysFont(font, 150).render(nameofworld, True,
                                                           (255 - color[0], 255 - color[1], 255 - color[2]))
        pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]), (
        (pxl_width - writething.get_width()) // 2 - 5, (pxl_height - writething.get_height()) // 5 - 5,
        writething.get_width() + 10, writething.get_height() + 10))
    ...
    if whereru == "saved":
        ...
        if selectedworld != -1:
            button(10, pxl_height - pxl_height / 2.5, (pxl_width - 30) / 2, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Delete",
                   "global selectedworld, worldlist ;os.remove('saves/'+worldlist[selectedworld]+'.labyrinth');selectedworld = -1",
                   80)
            button(20 + (pxl_width - 30) / 2, pxl_height - pxl_height / 2.5, (pxl_width - 30) / 2, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font,
                   "Play",
                   "global whereru,selectedworld; whereru = 'play'; getsaved(worldlist[selectedworld]+'.labyrinth');selectedworld = -1",
                   80)
        button(pxl_width / 2 - 500 / 2, pxl_height - pxl_height / 5, 500, 100,
               (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Back",
    ...
    if whereru == "tut":
        ...
        playersingleplayer()
        if int(tutstage) == 0:
            screen.blit(pygame.font.SysFont(font, 50).render("Move with WASD or the arrowkeys", True, (251, 72, 196)),
                        (0, 0))
            screen.blit(
                pygame.font.SysFont(font, 50).render(str(round(tutstage * 10, 1)) + "/10", True, (251, 72, 196)),
                (0, 80))
            if pygame.key.get_pressed()[pygame.K_d] or pygame.key.get_pressed()[pygame.K_RIGHT] or \
                    pygame.key.get_pressed()[pygame.K_w] or pygame.key.get_pressed()[pygame.K_UP] or \
                    pygame.key.get_pressed()[pygame.K_a] or pygame.key.get_pressed()[pygame.K_LEFT] or \
                    pygame.key.get_pressed()[pygame.K_s] or pygame.key.get_pressed()[pygame.K_DOWN]:
                tutstage += 0.001
    ...
    if whereru == "play":
        ...
        if legit:
            if zoom:
                if not inhole:
                    ...
                    for j in range(len(l)):
                        if holes.count(j) != 0 and (j % width) < x + 2 and (j % width) > x - 3 and (
                                j // width) < y + 2 and (j // width) > y - 3:
                            screen.blit(pygame.transform.scale(holeimg, (ZOOM_FB, ZOOM_FB)), (
                            (2.5 + (j % width - x)) * ZOOM_FB + BreiVer, (2.5 + (j // width - y)) * ZOOM_FB + HohVer))
                    ...
                    # minimap
                    pygame.draw.rect(screen, (0, 0, 0),
                                     (pxl_width - ZOOM_FB - 10, pxl_height - ZOOM_FB - 10, ZOOM_FB, ZOOM_FB), 0, 4, 4,
                                     4, 4)
                    pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]),
                                     (pxl_width - ZOOM_FB - 10, pxl_height - ZOOM_FB - 10, ZOOM_FB, ZOOM_FB), 3, 4, 4,
                                     4, 4)

                    pygame.draw.circle(screen, (255, 255, 255), (
                    pxl_width - ZOOM_FB - 10 + x / width * ZOOM_FB, pxl_height - ZOOM_FB - 10 + y / width * ZOOM_FB),
                                       max(ZOOM_FB / 20, 5))
                    for i in dogpos:
                        pygame.draw.circle(screen, (255, 0, 0), (pxl_width - ZOOM_FB - 10 + i[0] / width * ZOOM_FB,
                                                                 pxl_height - ZOOM_FB - 10 + i[1] / width * ZOOM_FB),
                                           max(ZOOM_FB / 30, 5))
                ...
                pygame.draw.circle(screen, (color[0], color[1], color[2]), (x * FB + BreiVer, y * FB + HohVer), 6 * FB,
                                   int(2.25 * FB))
                if yourtime > 10 and mousepoweractivated.gettime() > 45:
                    for i in dogpos:
                        dogs[0].draw_at(i[0], i[1], dogrichtungen[dogpos.index(i)])
        ...
        screen.blit(
            pygame.font.SysFont(font, 50).render(str(yourtime), True, (255 - color[0], 255 - color[1], 255 - color[2])),
            (0, 0))
    ...
    if whereru == "pause":
        ...
        if pausewru == "main":
            ...
            button(pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 1.8, pxl_width - 20, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Go to Main",
                   "global whereru,quitaction\nif asktosave: whereru = 'quit';quitaction = " + '"' + "global whereru;whereru = 'main'" + '"\nelse: whereru = "main"',
                   80)
            button(pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 1.4, pxl_width - 20, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Settings",
                   "global pausewru;pausewru = 'settings'", 80)
            button(10, pxl_height - pxl_height / 1.15, pxl_width / 2 - 15, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Help",
                   "webbrowser.open(r'https://e2z1.ml/projects/KittyLabyrinth')", 80)
            button(pxl_width / 2 + 5, pxl_height - pxl_height / 1.15, pxl_width / 2 - 15, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Tutorial",
        ...
        if pausewru == "settings":
            button(pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 1.15, pxl_width - 20, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Audio",
                   "global pausewru;pausewru = 'audio'", 80)
            button(pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 1.8, pxl_width - 20, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Look",
                   "global pausewru;pausewru = 'textures'", 80)
            button(pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 1.4, pxl_width - 20, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Debug",
                   "global pausewru;pausewru = 'debug'", 80)
            button(pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 5, pxl_width - 20, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font, "Done",
                   "global whereru,lastwhereru;whereru = lastwhereru[-2]", 80)
        ...
        if pausewru == "debug":
            ...
            button(pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 1.15, pxl_width - 20, 100,
                   (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font,
                   "Mousecontrol: " + str(mousecontrol), "global mousecontrol;mousecontrol = not mousecontrol", 80)
        ...
        if pausewru == "colors":
            ...
            screen.blit(pygame.font.SysFont(font, 50).render("R: " + str(color[0]), True,
                                                             (255 - color[0], 255 - color[1], 255 - color[2])), ((
                                                                                                                             pxl_width - pygame.font.SysFont(
                                                                                                                         font,
                                                                                                                         60).render(
                                                                                                                         ...
                                                                                                                         color[
                                                                                                                             2])).get_width()) // 2,
                                                                                                                 pxl_height - pxl_height / 2.3))
            pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]),
                             (pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 2.9, pxl_width - 20, 10),
                             0, 4, 4, 4, 4)
            pygame.draw.circle(screen, (255 - color[0], 255 - color[1], 255 - color[2]), (
            (color[0] / 255 * (pxl_width - 20) + (pxl_width / 2 - (pxl_width - 20) // 2)) - 20,
            pxl_height - pxl_height / 2.9 + 5), 20)

            pygame.draw.circle(screen, (255 - color[0], 255 - color[1], 255 - color[2]), (
            (color[1] / 255 * (pxl_width - 20) + (pxl_width / 2 - (pxl_width - 20) // 2)) - 20,
            pxl_height - pxl_height / 1.9 + 5), 20)
            pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]),
                             (pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 1.9, pxl_width - 20, 10),
                             0, 4, 4, 4, 4)
            screen.blit(pygame.font.SysFont(font, 50).render("G: " + str(color[1]), True,
                                                             (255 - color[0], 255 - color[1], 255 - color[2])), ((
                                                                                                                             pxl_width - pygame.font.SysFont(
                                                                                                                         font,
                                                                                                                         60).render(
                                                                                                                         ...
                                                                                                                         color[
                                                                                                                             2])).get_width()) // 2,
                                                                                                                 pxl_height - pxl_height / 1.6))

            pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]),
                             (pxl_width / 2 - (pxl_width - 20) // 2, pxl_height - pxl_height / 1.4, pxl_width - 20, 10),
                             0, 4, 4, 4, 4)
            pygame.draw.circle(screen, (255 - color[0], 255 - color[1], 255 - color[2]), (
            (color[2] / 255 * (pxl_width - 20) + (pxl_width / 2 - (pxl_width - 20) // 2)) - 20,
            pxl_height - pxl_height / 1.4 + 5), 20)
            screen.blit(pygame.font.SysFont(font, 50).render("B: " + str(color[2]), True,
                                                             (255 - color[0], 255 - color[1], 255 - color[2])), ((
                                                                                                                             pxl_width - pygame.font.SysFont(
                                                                                                                         font,
                                                                                                                         60).render(
            ...
            if onslider == 1:
                if m.get_pos()[0] < pxl_width - 26:
                    ...
                    changesettings("color[0]", str(int(
                        (m.get_pos()[0] + pxl_width / 2 - (pxl_width - 20) // 2) / (pxl_width - 20) * 255)))
            if onslider == 2:
                if m.get_pos()[0] < pxl_width - 26:
                    color[1] = int((m.get_pos()[0] + pxl_width / 2 - (pxl_width - 20) // 2) / (pxl_width - 20) * 255)
                    changesettings("color[1]", str(int(
                        (m.get_pos()[0] + pxl_width / 2 - (pxl_width - 20) // 2) / (pxl_width - 20) * 255)))
            if onslider == 3:
                if m.get_pos()[0] < pxl_width - 26:
                    color[2] = int((m.get_pos()[0] + pxl_width / 2 - (pxl_width - 20) // 2) / (pxl_width - 20) * 255)
                    changesettings("color[2]", str(int(
                        (m.get_pos()[0] + pxl_width / 2 - (pxl_width - 20) // 2) / (pxl_width - 20) * 255)))
        ...
        if pausewru == "audio":
            pygame.draw.rect(screen, (255 - color[0], 255 - color[1], 255 - color[2]),
                             ...
                             0, 4, 4, 4, 4)
            pygame.draw.circle(screen, (255 - color[0], 255 - color[1], 255 - color[2]), (
            (volume / 1 * (pxl_width - 20) + (pxl_width / 2 - (pxl_width - 20) // 2)) - 20,
            pxl_height - pxl_height / 1.4 + 5), 20)
            screen.blit(pygame.font.SysFont(font, 50).render("Volume: " + str(int(volume * 100)) + "%", True,
                                                             (255 - color[0], 255 - color[1], 255 - color[2])), ((
                                                                                                                             pxl_width - pygame.font.SysFont(
                                                                                                                         font,
                                                                                                                         60).render(
            ...
            if m.get_pressed()[0]:
                # wirdgepresst
                waspressed = True
                if pxl_height - pxl_height / 1.4 < m.get_pos()[
                    1] < pxl_height - pxl_height / 1.4 + 10 and onslider == 0:
                    onslider = 4
            ...
            if onslider == 4:
                if m.get_pos()[0] < pxl_width - 26:
                    ...
                    changesettings("volume",
                                   str((m.get_pos()[0] + pxl_width / 2 - (pxl_width - 20) // 2) / (pxl_width - 20)))
    ...
    if whereru == "win":
        ...
        for i in range(min(paths, 5)):
            screen.blit(pygame.transform.scale(glowf[fishmodel], ((pxl_width - 20) // 5, (pxl_width - 20) // 5)),
                        ((pxl_width - 20) // 5 * i, (pxl_height - 100) // 2 - (pxl_width - 20) // 5))
        button(10,
               (pxl_height - 100) // 2, pxl_width - 20, 100,
               (255 - color[0], 255 - color[1], 255 - color[2]), (color[0], color[1], color[2]), font,
        ...
        screen.blit(pygame.font.SysFont(font, 50).render(str(finaltime), True,
                                                         (255 - color[0], 255 - color[1], 255 - color[2])), (0, 0))
    if showfps:
